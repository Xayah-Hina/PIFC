...
if __name__ == '__main__':
    ...
    for ITERATION in range(1, 2):
        ...
        for _2, (batch_indices, batch_rays_o, batch_rays_d) in enumerate(tqdm.tqdm(sample_frustum(dirs=dirs, poses=poses, batch_size=batch_size, randomize=True, device=target_device))):
            ...
            raw_d = model_d(hidden)
            raw_flat = torch.zeros([batch_size * depth_size], device=target_device, dtype=torch.float32)
            ...
            dists_final = dists_cat * torch.norm(batch_rays_d[..., None, :], dim=-1)  # [batch_size, N_depths] // TODO: delete here
            ...
            img_loss = img2mse(rgb_map.clone(), batch_target_pixels)

            jac = torch.vmap(torch.func.jacrev(g))(hidden)
            jac_x = _get_minibatch_jacobian(hidden, batch_points_time_flat_filtered)
            jac = jac @ jac_x
