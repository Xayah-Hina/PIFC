...
class DensityOnlyExecutor:
    def __init__(self, batch_size: int, depth_size: int, ratio: float, target_device: torch.device, target_dtype: torch.dtype):
        ...
        self.scheduler = torch.optim.lr_scheduler.ExponentialLR(self.optimizer_d, gamma=gamma)
...
class HoudiniExecutor:
    ...
    def forward_1_iter(self):
        ...
        print(f"iter: {self.global_step}, new_lrate: {new_lrate}, nseloss_fine: {nseloss_fine}, img_loss: {10000 * img_loss}, proj_loss: {proj_loss}, min_vel_reg: {10.0 * min_vel_reg}, vel_loss: {vel_loss}")
...
def run_train_density_only():
    torch.set_float32_matmul_precision('high')
    executor = DensityOnlyExecutor(batch_size=1024, depth_size=192, ratio=0.5, target_device=torch.device("cuda:0"), target_dtype=torch.float32)
    pbar = tqdm.tqdm(desc="Running forward_1_iter", unit="iter")
    for _ in range(1000):
        executor.optimize_iter()
        pbar.update(1)
